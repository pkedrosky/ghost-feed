...
class GhostActivityPubEmbed extends HTMLElement {
  ...
  async fetchFeed() {
    ...
    try {
      ...
      const infiniteScrollEnabled = this.hasAttribute("infinite-scroll");
      const infiniteScrollSentinel =
        infiniteScrollEnabled && this._nextPage
          ? `<div class="infinite-scroll-sentinel" part="infinite-scroll-sentinel"></div>`
          : "";
  ...
  setupInfiniteScroll() {
    ...
    const sentinel = this.shadowRoot.querySelector(".infinite-scroll-sentinel");
    if (!sentinel) return;
