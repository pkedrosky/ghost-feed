...
class GhostActivityPubEmbed extends HTMLElement {
  ...
  static get observedAttributes() {
    return ["url", "proxy", "include-posts", "maxitems", "auto-load"];
  }
  ...
  async fetchFeed() {
    ...
    try {
      ...
      this.shadowRoot.querySelector(".feed-container").innerHTML =
        `${attachmentModal}${profileHeaderHTML}<div class="feed-items">${html}</div>`;
    } catch (error) {
      this.shadowRoot.querySelector(".feed-container").innerHTML =
  ...
  connectedCallback() {
    ...
    // Enable infinite scroll if auto-load attribute is present
    if (this.hasAttribute("auto-load")) {
      // Wait for initial feed to load before setting up observer
      setTimeout(() => {
        ...
        const feedItems = this.shadowRoot.querySelector(".feed-items");
        if (feedItems) {
          feedItems.appendChild(sentinel);
